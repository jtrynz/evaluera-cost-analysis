...
def _run_cost_estimate(sel_text, lot_size, avg_purchase, idf=None, supplier_col=None, item_col=None, selected_supplier=None):
    """
    OPTIMIERTE Kostenschätzung mit ALL-IN-ONE GPT-Call!

    VORHER: 2 separate GPT-Calls (Material + Fertigung)
    NACHHER: 1 kombinierter Call = 50% schneller & günstiger!

    - Lieferanten-Kompetenzen-Analyse (gecacht)
    - Material + Fertigung + Kosten (ALL-IN-ONE gecacht!)
    ...
    try:
        ...
        if idf is not None and not idf.empty and supplier_col and supplier_col in idf.columns:
            ...
            if supplier_name:
                ...
                # OPTIMIERT: Gecachter Call - vermeidet redundante API-Calls (Ladeanimation wird außen gezeigt)
                import json
                article_history_json = json.dumps(article_history) if article_history else ""
                supplier_competencies = cached_gpt_analyze_supplier(
                    supplier_name=supplier_name,
                    article_history_json=article_history_json,
                    country=country
                )
        ...
        # === ALL-IN-ONE KOSTENSCHÄTZUNG (Material + Fertigung in EINEM GPT-Call!) ===
        print(f"\n🔍 Starte ALL-IN-ONE Kostenschätzung für: '{sel_text}'")
        print(f"   Losgrösse: {lot_size:,} Stk")
        print(f"   {'MIT Lieferanten-Kompetenzen' if supplier_competencies else 'OHNE Lieferanten-Daten'}")

        # OPTIMIERT: Kombinierter Call - 50% schneller & günstiger als 2 separate Calls! (Ladeanimation wird außen gezeigt)
        import json
        supplier_comps_json = json.dumps(supplier_competencies) if supplier_competencies else None
        ...
        _show_gpt_error(result, 'All-in-One Kostenschätzung')

        # Extrahiere alle Werte aus dem kombinierten Result
        mat = result.get('material_guess', 'stahl')
        mass_kg = result.get('mass_kg')
        d_mm = result.get('d_mm')
        l_mm = result.get('l_mm')
        material_eur = result.get('material_cost_eur')
        eur_per_kg = result.get('material_price_eur_kg')
        fab_eur = result.get('fab_cost_eur')
        process = result.get('process', 'unknown')
        confidence = result.get('confidence', 'unknown')
        assumptions = result.get('assumptions', [])

        print(f"\n✅ ALL-IN-ONE Schätzung abgeschlossen!")
        print(f"   → Material: {mat}, Masse: {mass_kg:.6f} kg" if mass_kg else f"   → Material: {mat}")
        print(f"   → Geometrie: d={d_mm:.1f}mm, l={l_mm:.1f}mm" if d_mm and l_mm else "   → Geometrie: N/A")
        print(f"   → Prozess: {process}")
        print(f"   → Materialkosten: {material_eur:.4f} €" if material_eur else "   → Materialkosten: N/A")
        print(f"   → Fertigungskosten: {fab_eur:.4f} €" if fab_eur else "   → Fertigungskosten: N/A")
        ...
        # Fallback: Materialkosten berechnen falls GPT keine liefert
        if material_eur is None and mass_kg and eur_per_kg:
            material_eur = mass_kg * eur_per_kg
            print(f"   → FALLBACK: material_eur berechnet = {material_eur:.4f} €")

        # Legacy-Kompatibilität: part_class (nicht in neuer Funktion, aber in Return erwartet)
        part_class = 'standard'  # Default-Wert
        ...
        return {
            ...
            'gpt_complete_raw': result,  # NEU: Kombiniertes Result statt 2 separate
...
with col_theme:
    ...
    # Verbesserter Theme-Toggle Button mit Icon
    current_theme = "Dark" if st.session_state.dark_mode else "Light"
    next_theme = "Light" if st.session_state.dark_mode else "Dark"
    theme_icon = "☀️" if st.session_state.dark_mode else "🌙"

    if st.button(f"{theme_icon} {next_theme}", key="theme_toggle", help=f"Wechsel zu {next_theme} Mode"):
    ...
    **Funktion:**
    ...
    if cad_3d_up is not None:
        ...
        if st.button("🚀 Kosten für 3D-Modell schätzen", type="primary", use_container_width=True, key="cad_3d_cost_btn"):
            ...
            # === OPTIMIERT: Excel-Durchsuchung für 3D-Modell ===
            cad_3d_avg_price = None

            if cad_3d_excel_up is not None:
                try:
                    cad_3d_excel_df = read_and_normalize_excel(cad_3d_excel_up)
                    matches, cad_3d_avg_price, cad_3d_mn, cad_3d_mx = search_excel_for_article(
                        cad_3d_excel_df, cad_3d_description, use_gpt=True
                    )

                    if matches is not None:
                        display_excel_matches(matches, cad_3d_avg_price, cad_3d_mn, cad_3d_mx,
                                            context=" für 3D-Modell")
                    else:
    ...
    if drawing_up is not None:
        with GPTLoadingAnimation("📐 Analysiere Zeichnung...", icon="🤖"):
            ...
            if drawing_data.get("ok"):
                ...
                if items:
                    ...
                    if st.button("🚀 Kosten für CAD-Artikel schätzen", type="primary", use_container_width=True):
                        ...
                        # === OPTIMIERT: Excel-Durchsuchung für CAD-Artikel ===
                        cad_avg_price = None

                        if drawing_excel_up is not None:
                            try:
                                cad_excel_df = read_and_normalize_excel(drawing_excel_up)
                                matches, cad_avg_price, cad_mn, cad_mx = search_excel_for_article(
                                    cad_excel_df, cad_description, use_gpt=True
                                )

                                if matches is not None:
                                    display_excel_matches(matches, cad_avg_price, cad_mn, cad_mx)
                                else:
...
if not cad_only_mode:
    ...
    with left:
        ...
        price_series = None  # Initialisiere price_series für spätere Verwendung
        if not idf.empty:
            try:
                avg, mn, mx, _qcol, _src = derive_unit_price(idf)

                # Berechne price_series für spätere Verwendung
                ...
                if supplier_col and supplier_col in idf.columns and num_suppliers_in_results > 1:
                    with st.expander(f"📋 Preis-Breakdown nach Lieferant ({num_suppliers_in_results} Lieferanten)", expanded=True):
                        # Gruppiere nach Lieferant (price_series wurde bereits oben berechnet)
        ...
        if not idf.empty and supplier_col and supplier_col in idf.columns:
            ...
            if len(available_suppliers) > 1:
                ...
                # Debug: Zeige verfügbare Lieferanten
                with st.expander("📋 Verfügbare Lieferanten", expanded=False):
                    for i, sup in enumerate(available_suppliers, 1):
                        st.write(f"{i}. {sup}")
                ...
                # Session State für Lieferanten-Auswahl initialisieren
                if "selected_supplier_name" not in st.session_state:
                    st.session_state.selected_supplier_name = None

                # Dropdown für Lieferanten - DIREKTE String-Liste
                supplier_options = ["(Bitte Lieferant wählen...)"] + available_suppliers

                # Finde den Index des zuvor gewählten Lieferanten
                default_index = 0
                if st.session_state.selected_supplier_name and st.session_state.selected_supplier_name in available_suppliers:
                    default_index = supplier_options.index(st.session_state.selected_supplier_name)
                ...
                if selected_supplier != "(Bitte Lieferant wählen...)":
                    st.session_state.selected_supplier_name = selected_supplier

                    # DEBUG: Zeige was gespeichert wurde
                    st.caption(f"💾 Gespeichert in Session: {st.session_state.selected_supplier_name}")
                else:
                    selected_supplier = ""
                    st.session_state.selected_supplier_name = None

                if selected_supplier and selected_supplier != "":
                    # Zeige Info über gewählten Lieferanten in hervorgehobener Box
                    ...
                    # Hervorgehobene Bestätigung der Auswahl - GRÖSSER und DEUTLICHER
                    st.markdown(f"""
                    <div style='background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%);
                        ...
                        <div style='display: flex; align-items: center; gap: 1.5rem;'>
                            ...
                            <div style='flex: 1;'>
                                <div style='font-weight: 700; font-size: 1.3rem; margin-bottom: 0.5rem;'>
                                    Ausgewählter Lieferant: {selected_supplier}
                                </div>
                                <div style='font-size: 1rem; opacity: 0.95; display: flex; gap: 2rem;'>
                                    <span>💰 Ø Preis: {supplier_avg_price:,.4f} €</span>
                                    <span>📦 {len(supplier_data)} Einträge</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    st.warning("⚠️ Bitte wähle einen Lieferanten aus dem Dropdown oben aus.")
            elif len(available_suppliers) == 1:
                # Nur ein Lieferant verfügbar
                selected_supplier = available_suppliers[0]
                st.session_state.selected_supplier_name = selected_supplier
                st.success(f"🏭 **Einziger verfügbarer Lieferant:** {selected_supplier}")
        ...
        # Nutze Session State als primäre Quelle
        final_supplier_for_analysis = selected_supplier
        if not idf.empty and supplier_col and supplier_col in idf.columns:
            available_suppliers = sorted(idf[supplier_col].dropna().unique().tolist())
            if len(available_suppliers) > 1 and (not selected_supplier or selected_supplier == ""):
                ...
                st.info(f"💡 **Kein Lieferant gewählt** → Analysiere mit häufigstem Lieferanten: **{default_supplier}** (wähle oben einen anderen für spezifische Analyse)")
                final_supplier_for_analysis = default_supplier

        # Debug: Zeige welcher Lieferant für Analyse verwendet wird
        if final_supplier_for_analysis:
            st.caption(f"🔍 Lieferant für Analyse: **{final_supplier_for_analysis}**")

        if st.button("🚀 Kosten schätzen (GPT + Lieferanten-Analyse)", type="primary", use_container_width=True):
            if sel:
                # Professionelle Ladeanimation während GPT-Analyse
                with GPTLoadingAnimation("🤖 Analysiere Material- und Fertigungskosten mit KI...", icon="💰"):
                    st.session_state.est_result = _run_cost_estimate(
                        ...
                        item_col=item_col,
                        selected_supplier=final_supplier_for_analysis  # Verwende finalen Lieferanten
                    )

                # Erfolgsmeldung nur bei erfolgreicher Schätzung
                if st.session_state.est_result and st.session_state.est_result.get("ok"):
                    show_gpt_success("✅ Kostenschätzung erfolgreich abgeschlossen!")
            else:
        ...
        if res is None:
            ...
            if not res.get("ok"):
                ...
                # API-Usage Tracking anzeigen (ALL-IN-ONE Format)
                gpt_complete_raw = res.get("gpt_complete_raw", {})

                # ALL-IN-ONE: Nur ein API-Call statt zwei
                api_called = gpt_complete_raw.get("_api_called", False)
                fallback = gpt_complete_raw.get("_fallback", False)
                tokens_used = gpt_complete_raw.get("_tokens_used", 0)
                ...
                if fallback:
                    ...
                    **Lösung:** Check Konsole-Output für Details!
                    ...
                    - **ALL-IN-ONE Schätzung:** Material + Fertigung in einem Call!
                    - **Tokens verwendet:** {tokens_used}
                    - **Ersparnis:** 50% weniger Tokens als vorher (2 Calls)

                    💰 **Geschätzte Kosten:** ~${tokens_used * 0.00002:.6f} USD
                ...
                if mn is not None and mx is not None and mn < mx:
                    # Sanity-Check: Filtere unrealistische Preise (< 0.001 €)
                    if mn < 0.001:
                        st.warning(f"⚠️ Der minimale Preis ({mn:.6f} €) ist unrealistisch niedrig. Möglicherweise Datenfehler. Ersparungsberechnung übersprungen.")
                    else:
                        st.markdown("---")
            ...
            if mass_kg and mass_kg > 0:
                ...
                # CBAM nur für Nicht-EU-Lieferanten anzeigen
                if not co2_result.get('is_eu_source', True):
                    st.markdown("---")
                        ...
                        with st.expander(title, expanded=False):
                            ...
                            if not rating_result:
                                ...
                                if st.button(f"🤖 Lieferant analysieren", key=f"analyze_supplier_{idx}", type="primary", use_container_width=True):
                                    with GPTLoadingAnimation(f"🔍 Analysiere Lieferant...", icon="🤖"):
                    ...
                    if ratings:
                        ...
                        if st.button("📋 Verhandlungsstrategie generieren", key="nego_prep_btn"):
                            ...
                            if not rating_result:
                                ...
                                with GPTLoadingAnimation("🎯 Erstelle Strategie...", icon="🤖"):
