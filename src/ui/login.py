...
def render_login_screen():
    ...
    <style>
        ...
        .login-container .stTextInput > div > div > input:focus {{
            ...
            box-shadow: 0 0 0 4px {COLORS['surface_tint']}, 0 2px 8px rgba(0,0,0,0.08) !important;
        ...
        .login-container .stButton > button {{
            width: 100% !important;
            background: linear-gradient(135deg, {COLORS['primary']} 0%, {COLORS['error']} 100%) !important;
...
def render_logout_button():
    ...
    with st.sidebar:
        ...
        <div style="
            padding: 16px;
            background: linear-gradient(135deg, {COLORS['surface_tint']} 0%, {COLORS['primary_light']} 100%);
...
def inject_lottie_background():
    """Inject Lottie animation background for login screen"""
    # Get path to dark_gradient.json
    lottie_path = os.path.join(os.path.dirname(__file__), '..', '..', 'assets', 'dark_gradient.json')

    # Try to load Lottie JSON
    lottie_json = "{}"
    ...
    st.markdown(f"""
    <style>
        /* Fullscreen Lottie Background (Login Only) */
        #lottie-background {{
            position: fixed;
            top: 0;
            left: 0;
        ...
        /* Hide on non-login screens */
        [data-testid="stSidebar"] ~ div #lottie-background {{
            display: none;
        }}

        /* Prevent scroll on login screen */
